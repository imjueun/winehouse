...
orderRouter.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    // 상품 목록 호출
    const orderlistOfOrder = await OrderlistService.getOrderlistOfOrder(
      orderId
    );
    // 성공 상태 핸들링
    res.status(200).json({
...
orderRouter.get(
  ...
  asyncHandler(async (req, res, next) => {
    const { orderId } = req.params;
    // 배송 상태 호출
    const getDeliveryOfOrder = await DeliveryService.getDeliveryOfOrder(
      orderId
    );
    // 성공 상태 핸들링
    res.status(200).json({
...
orderRouter.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    // deliverySchema 추가
    const addDelivery = await DeliveryService.addDelivery(
      req.userId,
      addOrder._id
    );
    // 상품 총 수량 및 금액 계산
    const getOrderQuntity = await OrderlistService.getOrderQuntity(
      addOrder._id
    );
    // orderSchmea에 총 가격 및 수량 기입
    const setOrder = await OrderService.setOrder(
      addOrder._id,
      getOrderQuntity.orderPrice,
      getOrderQuntity.orderNum
    );
    // 성공 상태 핸들링
    res.status(201).json({
      status: 201,
...
orderRouter.patch(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    // 배송 상태 수정 함수 호출
    const updateDelivery = await DeliveryService.setDelivery(
      deliveryId,
      req.body
    );
    // 성공 상태 핸들링
