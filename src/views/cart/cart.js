...
deleteSelectedButton.addEventListener('click', () => {
  const checkedItems = document.querySelectorAll(
    '#cart-items input[type="checkbox"]:checked'
  );
...
function calculateCheckedItemsTotal() {
  ...
  cartItems.forEach((checkbox) => {
    ...
    const itemQuantity = parseInt(
      itemCard.querySelector('.quantity').innerText
    );
...
function updateTotalPrice() {
  ...
  const shippingCost =
    checkedItemsTotal > 0 ? (checkedItemsTotal >= 50000 ? 0 : 5000) : 0;
      ...
      if (confirmDelete) {
        ...
        checkedItems.forEach((checkbox) => {
          ...
          const itemPrice = parseInt(
            itemPriceElement.innerText.replace('원', '')
          );
          // 수량
          const itemQuantity = parseInt(
            itemCard.querySelector('.quantity').innerText
